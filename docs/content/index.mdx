...
<Callout type="tip">
  **Committed branches are never rewritten mid-run.** NIKI runs the pipeline inside an isolated container sandbox, debates the implementation, executes real test suites, and hands you a clean branch ready for human review — the host working tree receives the finished diff for review.
</Callout>
