---
title: CLI Overview
description: Complete inventory of all 18 NIKI CLI commands.
sidebar:
  order: 1
  label: CLI Overview
---
...
NIKI ships with 18 top-level CLI commands organized into 4 logical categories:
...
* **`niki research query <q>`**: Autonomous web search & cited technical summary generator.

---

## 4. Interactive TUI, IDE Integration, Benchmarking & Verification
* **`niki chat`**: Interactive terminal chat interface with streaming markdown and slash commands (the default when invoked bare).
* **`niki acp`**: Run NIKI as an Agent Client Protocol (ACP) JSON-RPC server over stdio for IDEs like Zed (`--project <dir>`).
* **`niki voice`**: Record a push-to-talk clip via `ffmpeg` and transcribe it through your provider's STT endpoint (`--seconds`, `--language`).
